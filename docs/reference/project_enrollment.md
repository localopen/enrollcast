...
grade progression ratio method. Internally builds a Leslie matrix from
`ratios` and advances enrollment one year at a time (one matrix-vector
product per projected year), overwriting the entry grade with the
supplied exogenous value each year.
