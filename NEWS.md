...
* `progression_ratios()` validates column selectors, historical enrollment and
  years, warns when calendar-year gaps are skipped, and supports mean,
  geometric, median, last, and strictly validated weighted summaries.
* `project_enrollment()` validates base and entry enrollment, year labels, and
  ...
  coefficients are preserved with an aggregate warning.
